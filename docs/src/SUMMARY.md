# Summary

-   [Introduction to the dev. bootcamp](intro/overview.md)

    -   [Format Overview](intro/format-overview.md)
    -   [Deliverables](intro/deliverables.md)
    -   [Sessions](intro/sessions/overview.md)

        -   [Session 1](intro/sessions/session-1.md)
        -   [Session 2](intro/sessions/session-2.md)
        -   [Session 3](intro/sessions/session-3.md)
        -   [Session 4](intro/sessions/session-4.md)

-   [Initial Setup](setup/overview.md)

    -   [Windows](setup/windows.md)
    -   [Mac](setup/mac.md)
    -   [Linux](setup/linux.md)

-   [Terminals, Code Editors, and You](terminals-and-editors/overview.md)

    -   [Quick Terminal Overview](terminals-and-editors/terminal.md)
    -   [Editing Code with Ease](terminals-and-editors/editors.md)
    -   [Pimp my VSCode](terminals-and-editors/pimp.md)

-   [Git, and GitHub](git/overview.md)

    -   [Why bother using git?](git/why-bother.md)
    -   [How does git even work?](git/internals.md)

-   [Web Server](web/overview.md)

    -   [HTTP](web/http.md)
    -   [Flask](web/flask/overview.md)

        -   [Example 1 - Hello, Flask!](web/flask/example-1.md)
        -   [Example 2 - Flask Math](web/flask/example-2.md)
        -   [Example 3 - Rainbow Flask](web/flask/example-3.md)

-   [Example Portfolio](example-portfolio/overview.md)

    -   [Running the application locally](example-portfolio/running-locally.md)
    -   [Adding new blog posts](example-portfolio/adding-new-blog-posts.md)
